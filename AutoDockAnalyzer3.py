...
# ==============================================================================
# PLUGIN: AutoDockAnalyzer 3 (ADA 3)
# VERSION: 3.1
# DATE: 2026-07-02
...
FEATURES:
    - Tab 1 (AutoDock4): Greedy clustering, medoid generation, statistical calculations (Kd, pKd),
...
try:
    ...
    HAS_MATLIB = False  # If not present, save the state for GUI

#Plugin Version

version = 3.1

# Personal Logo Data (JGM logo encoded in Base64)
...
class ADA:
    ...
    def load_dlg(self, filename):
        ...
        print(f"[ADA 3] Loaded {len(self.poses)} Classic poses successfully.")
        return True
    ...
    def create_clusters(self):
            cmd.set("suspend_updates", 1)  # Disable screen rendering
    ...
    def plot(self):
        """
        Generates a dual-axis bar chart showing both cluster population
        and energy standard deviation as error bars using a non-blocking backend.
        """
        table = self.table()
        if not table:
            return
        ...
        # DEFERRED IMPORTS & NON-BLOCKING BACKEND
        import matplotlib
        matplotlib.use('Agg', force=True)
        import matplotlib.pyplot as plt

        # Extraer los datos necesarios de la tabla recalculada
        cluster_ids = [row[0] for row in table]
        populations = [row[1] for row in table]
        # Si la SD es None o0 (clúster de 1 sola pose), le asignamos 0.0 para el gráfico
        energy_sds = [row[6] if (row[6] is not None) else 0.0 for row in table]

        fig, ax1 = plt.subplots(figsize=(8, 5))

        # EJE Y1 (Izquierda): Población del Clúster (Barras principales)
        color_bars = '#3498db'  # Azul elegante
        bars = ax1.bar(cluster_ids, populations, color=color_bars, alpha=0.7, label='Population')
        ax1.set_xlabel("Cluster ID", fontweight='bold')
        ax1.set_ylabel("Population (# poses)", color=color_bars, fontweight='bold')
        ax1.tick_params(axis='y', labelcolor=color_bars)
        ax1.set_xticks(cluster_ids)  # Asegura que se muestren todos los IDs de clúster

        # EJE Y2 (Derecha): Desviación Estándar de Energía (Como Barras de Error superpuestas)
        # Creamos un eje gemelo que comparte el mismo eje X
        ax2 = ax1.twinx()
        color_errors = '#e74c3c'  # Rojo para destacar el error estadístico

        # Dibujamos las barras de error invisibles en su base pero con "caps" visibles en color rojo
        # Usamos las posiciones de las barras anteriores para centrar las líneas de error
        ax2.errorbar(cluster_ids, energy_sds, yerr=None, fmt='none', ecolor=color_errors,
                     elinewidth=2, capsize=5, markeredgewidth=2, label='Energy SD')

        # Adicionalmente, conectamos los puntos de SD con una línea discontinua suave para ver la tendencia
        ax2.plot(cluster_ids, energy_sds, color=color_errors, linestyle='--', marker='o', alpha=0.5)

        ax2.set_ylabel("Energy SD (kcal/mol)", color=color_errors, fontweight='bold')
        ax2.tick_params(axis='y', labelcolor=color_errors)

        # Configurar límites mínimos para estética (evitar valores negativos en SD)
        ax2.set_ylim(bottom=0)

        plt.title("Cluster Population & Energy Homogeneity (SD)", fontsize=12, fontweight='bold', pad=15)
        fig.tight_layout()  # Ajusta los márgenes para que no se corten los textos de los ejes

        # PIPELINE DE GUARDADO ASÍNCRONO (No bloqueante)
        output_path = os.path.join(os.path.expanduser("~"), "ada_cluster_plot.png")
        fig.savefig(output_path, dpi=150)
        plt.close(fig)

        # Lanzar visor nativo del Sistema Operativo
        import platform
        import subprocess
    ...
    def plot(self):
        ...
        """
        # Recalculate and fetch the latest statistical table data from the clusters
        table = self.table()
        if not table: 
            return
        
        # Guard clause: Verify if matplotlib is available before attempting execution
        if not HAS_MATPLOTLIB:
            print("[ADA] Error: Matplotlib is not installed. Plotting functionality is disabled.")
            return
        ...
        import matplotlib
        # Force a non-interactive backend ('Agg') to prevent GUI thread conflicts with PyMOL
        matplotlib.use('Agg', force=True) 
        import matplotlib.pyplot as plt
        
        # Extract individual metric arrays from the cluster table rows
        cluster_ids = [row[0] for row in table]
        populations = [row[1] for row in table]
        
        # Extract Energy SD (index 6); fallback to 0.0 if None (e.g., single-pose clusters)
        energy_sds = [row[6] if (row[6] is not None) else 0.0 for row in table]
        
        # Initialize the figure canvas and subplot axes
        fig, ax = plt.subplots(figsize=(8, 5))
        
        # Graphical palette definition
        color_bars = '#3498db'    # Clean flat blue for population frequencies
        color_errors = '#e74c3c'  # Sharp flat red for thermodynamic dispersion (SD)
        ...
        # Set standardized structural formatting for axes, padding, and tick marks
        ax.set_xlabel("Cluster ID", fontweight='bold', labelpad=10)
        ax.set_ylabel("Cluster Population (# poses)", fontweight='bold', labelpad=10)
        ax.set_xticks(cluster_ids)  # Force explicit rendering for discrete integer IDs
        
        # Set chart title with appropriate padding and styling
        ax.set_title("Cluster Population with Energy SD as Error Bars", fontsize=12, fontweight='bold', pad=15)
        
        # Automatically adjust layout bounding geometry to avoid overlapping labels
        fig.tight_layout()
        
        # ASYNCHRONOUS EXPORT: Save the rendered canvas to a temporary disk location
        output_path = os.path.join(os.path.expanduser("~"), "ada_cluster_plot.png")
        fig.savefig(output_path, dpi=150)
        plt.close(fig)  # Immediately clear memory buffers and tear down the figure
        
        # Platform-specific pipeline dispatch to spawn a non-blocking system image viewer
        import platform
        import subprocess
        if platform.system() == "Windows":
            os.startfile(output_path)
    ...
    def export_csv(self, filename):
        with open(filename, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["Cluster", "Size", "Best Energy", "Kd (M)", "pKd", "Mean Energy", "Standard Deviation"])
...
class GUI(QtWidgets.QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"AutoDockAnalyzer v{version}")
        self.resize(700, 580)
    ...
    def init_autodock_tab(self):
        ...
        # DYNAMIC UI HANDLING: Disable button if dependencies are not met
        if not HAS_MATPLOTLIB:
    ...
    def init_acknowledgements_tab(self):
        ...
        # Ensure that opening external links is enabled if necessary
    ...
    def cluster_classic(self):
        ...
        self.table_widget.setHorizontalHeaderLabels(["Cluster", "Size", "Best Energy", "Kd (M)", "pKd", "Mean Energy", "Standard Deviation"])
