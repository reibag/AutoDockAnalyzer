...
try:
    ...
    HAS_MATLIB = False  # Si no está, guardamos el estado para proteger la GUI

#Plugin Version

version = 3.1

# Personal Logo Data (image_9440e0.png encoded in Base64)
...
class ADA:
    ...
    def create_clusters(self):
            cmd.set("suspend_updates", 1)  # Suspende el renderizado de la pantalla
            ...
            cmd.set("suspend_updates", 0)  # Reactiva y refresca todo de golpe
    ...
    def plot(self):
        """
        Generates a bar chart of the cluster populations.
        Uses lazy loading and a non-blocking backend to prevent PyMOL GUI freezing.
        """
        table = self.table()
        if not table: 
            return
        
        # Fallback guard if the library is missing from the environment
        if not HAS_MATPLOTLIB:
            print("[ADA 3] Error: Matplotlib is not installed. Plotting functionality is disabled.")
            return

        # DEFERRED IMPORTS: Loaded here to avoid startup dependency issues
        import matplotlib
        # Force a non-interactive backend ('Agg') to prevent Qt thread collisions with PyMOL
        matplotlib.use('Agg', force=True) 
        import matplotlib.pyplot as plt
        
        fig, ax = plt.subplots()
        ax.bar([row[0] for row in table], [row[1] for row in table])
        ax.set_xlabel("Cluster ID")
        ax.set_ylabel("Population (#poses)")
        ax.set_title("Cluster Population")
        
        # ASYNCHRONOUS VIEWING: Save to a temporary file instead of calling blocking 'plt.show()'
        output_path = os.path.join(os.path.expanduser("~"), "ada_cluster_plot.png")
        fig.savefig(output_path, dpi=150)
        plt.close(fig)
        
        # Open the generated image using the operating system's native default viewer
        import platform
        import subprocess
